...
//           input stream is a file input stream, with words separated by whitespace.
...
double reviewScore(const vector<string> &reviewWords, const vector<string> &keywords, const vector<double> &weights) {
    vector<string> reviewWords_C = reviewWords;
    double currentWeight = 0;
    double totalWeight = 0;

    preprocessReview(reviewWords_C);
    for (int i = 0; i < reviewWords_C.size(); ++i) {
        currentWeight = wordWeight(reviewWords_C[i], keywords, weights); // calculating currentWeight using wordWeight function
        totalWeight += currentWeight;                                    // Updating totalWeight as we go through the loop
